        ...
        type : 'POST',
            ...
            success : function(data){
              if (data.texts.length === 0) {
               ...
               elements += '<div class="form-group"><label for="category_details">{{ trans("Admin/place.place_discription") }}</label><textarea  cols="5" rows="5" class="form-control" id="info_description_'+i+'" name="info_description[]" placeholder="Enter  Details" ></textarea></div>';
             ...
             $('#add-texts').append('<div class="form-group"><button type="button" class="btn btn-primary add-another-place-details" >+</button></div>');

             $('#add-texts').append('<div class="modal-footer"><button type="button" class="btn btn-default" data-dismiss="modal">Close</button><input id="tag-form-submit" type="submit" class="btn btn-primary" value="Save"></div>');
           }
         });
    });
...
$("#add-texts").on("click", ".add-another-place-details", function(){
  ...
  if ($("#info_title_" + row_index).val() && $("#info_description_" + row_index).val()) {

                var i = parseInt(row_index) + 1;

                var elements = '<div class="card-body" id="'+i+'">'; 
                elements += '<div class="form-group"><label for="place_name">{{ trans("Admin/place.place_name") }}<span class="text-danger">*</span></label><div id="place_name_'+i+'">'+placename+'</div><label for="category_name">{{ trans("Admin/place.place_title") }}<span class="text-danger">*</span></label>';

               elements += '<input type="text" class="form-control" id="info_title_'+i+'" name="info_title[]" placeholder="Enter Title" value="">';

               elements += '<input type="hidden" class="form-control" id="place_id_'+i+'" name="place_id[]" value="'+placeid+'"></div>';
            ...
            $.ajax({
                ...
                success: function(result) {
                   if(result.status == 200){
                    ...
                    {
                      $("#resmsg").html('<div class="alert alert-danger text-center" role="alert">Something Went Wrong</div>'); //show error message 
                    }
                },
